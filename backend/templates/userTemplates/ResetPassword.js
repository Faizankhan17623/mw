const ResetPasswordTemplate = (password) => {
	return `<!DOCTYPE html>
	<html lang="en">
	<head>
		<meta charset="UTF-8">
		<meta name="viewport" content="width=device-width, initial-scale=1.0">
		<meta http-equiv="X-UA-Compatible" content="IE=edge">
		<title>Password Reset - Cine Circuit</title>
		<style>
			/* Reset */
			body, table, td, a {
				-webkit-text-size-adjust: 100%;
				-ms-text-size-adjust: 100%;
			}
			table, td {
				mso-table-lspace: 0pt;
				mso-table-rspace: 0pt;
			}
			img {
				-ms-interpolation-mode: bicubic;
				border: 0;
				height: auto;
				line-height: 100%;
				outline: none;
				text-decoration: none;
			}

			body {
				margin: 0 !important;
				padding: 0 !important;
				background-color: #0f0f0f;
				font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
				-webkit-font-smoothing: antialiased;
			}

			.email-wrapper {
				width: 100%;
				background-color: #0f0f0f;
				padding: 40px 0;
			}

			.email-container {
				max-width: 520px;
				margin: 0 auto;
				background: linear-gradient(145deg, #1a1a2e 0%, #16213e 50%, #1a1a2e 100%);
				border-radius: 16px;
				overflow: hidden;
				box-shadow: 0 20px 60px rgba(0, 0, 0, 0.5), 0 0 40px rgba(229, 9, 20, 0.08);
				border: 1px solid rgba(255, 255, 255, 0.06);
			}

			/* Header */
			.header {
				background: linear-gradient(135deg, #e50914 0%, #b20710 100%);
				padding: 32px 40px;
				text-align: center;
			}

			.brand-name {
				font-size: 28px;
				font-weight: 800;
				color: #ffffff;
				letter-spacing: 2px;
				margin: 0;
				text-transform: uppercase;
			}

			.brand-tagline {
				font-size: 12px;
				color: rgba(255, 255, 255, 0.8);
				letter-spacing: 4px;
				text-transform: uppercase;
				margin-top: 6px;
			}

			/* Icon */
			.icon-section {
				text-align: center;
				padding: 32px 0 0 0;
			}

			.icon-circle {
				display: inline-block;
				width: 72px;
				height: 72px;
				background: linear-gradient(135deg, rgba(241, 196, 15, 0.15) 0%, rgba(241, 196, 15, 0.05) 100%);
				border-radius: 50%;
				line-height: 72px;
				border: 2px solid rgba(241, 196, 15, 0.3);
			}

			.icon-circle span {
				font-size: 32px;
			}

			/* Content */
			.content {
				padding: 24px 40px 20px 40px;
				text-align: center;
			}

			.title {
				font-size: 22px;
				font-weight: 700;
				color: #ffffff;
				margin: 0 0 8px 0;
			}

			.subtitle {
				font-size: 14px;
				color: rgba(255, 255, 255, 0.55);
				margin: 0 0 24px 0;
				line-height: 1.5;
			}

			.greeting {
				font-size: 15px;
				color: rgba(255, 255, 255, 0.75);
				text-align: left;
				margin: 0 0 16px 0;
				line-height: 1.6;
			}

			/* New Password Box */
			.password-section {
				padding: 0 40px 8px 40px;
				text-align: center;
			}

			.password-card {
				background: linear-gradient(135deg, rgba(241, 196, 15, 0.1) 0%, rgba(241, 196, 15, 0.04) 100%);
				border: 2px dashed rgba(241, 196, 15, 0.35);
				border-radius: 12px;
				padding: 24px 20px;
			}

			.password-label {
				font-size: 11px;
				font-weight: 600;
				color: rgba(255, 255, 255, 0.4);
				text-transform: uppercase;
				letter-spacing: 3px;
				margin: 0 0 14px 0;
			}

			.password-value {
				font-size: 28px;
				font-weight: 800;
				color: #f1c40f;
				letter-spacing: 4px;
				margin: 0;
				font-family: 'Courier New', monospace;
				word-break: break-all;
				text-shadow: 0 0 20px rgba(241, 196, 15, 0.25);
			}

			.password-hint {
				font-size: 12px;
				color: rgba(255, 255, 255, 0.35);
				margin: 14px 0 0 0;
			}

			/* Steps Section */
			.steps-section {
				padding: 20px 40px 8px 40px;
			}

			.steps-title {
				font-size: 11px;
				font-weight: 700;
				color: rgba(255, 255, 255, 0.35);
				text-transform: uppercase;
				letter-spacing: 3px;
				margin: 0 0 16px 0;
				text-align: center;
			}

			.step-item {
				display: table;
				width: 100%;
				padding: 12px 0;
				border-bottom: 1px solid rgba(255, 255, 255, 0.04);
			}

			.step-item:last-child {
				border-bottom: none;
			}

			.step-number {
				display: table-cell;
				width: 32px;
				vertical-align: top;
			}

			.step-badge {
				display: inline-block;
				width: 26px;
				height: 26px;
				background: linear-gradient(135deg, rgba(229, 9, 20, 0.2) 0%, rgba(229, 9, 20, 0.1) 100%);
				border-radius: 50%;
				text-align: center;
				line-height: 26px;
				font-size: 12px;
				font-weight: 700;
				color: #e50914;
				border: 1px solid rgba(229, 9, 20, 0.3);
			}

			.step-text {
				display: table-cell;
				vertical-align: middle;
				padding-left: 12px;
				font-size: 13px;
				color: rgba(255, 255, 255, 0.6);
				line-height: 1.5;
			}

			/* Warning Section */
			.warning-section {
				padding: 16px 40px 32px 40px;
			}

			.warning-box {
				background: linear-gradient(135deg, rgba(231, 76, 60, 0.08) 0%, rgba(231, 76, 60, 0.03) 100%);
				border-radius: 10px;
				padding: 16px 20px;
				border-left: 3px solid rgba(231, 76, 60, 0.5);
			}

			.warning-box p {
				font-size: 13px;
				color: rgba(255, 255, 255, 0.55);
				margin: 0;
				line-height: 1.6;
				text-align: left;
			}

			.warning-box a {
				color: #e50914;
				text-decoration: none;
				font-weight: 600;
			}

			/* Security Tips */
			.tips-section {
				padding: 0 40px 28px 40px;
			}

			.tips-box {
				background: rgba(255, 255, 255, 0.03);
				border-radius: 10px;
				padding: 18px 20px;
				border: 1px solid rgba(255, 255, 255, 0.06);
			}

			.tips-title {
				font-size: 11px;
				font-weight: 700;
				color: rgba(255, 255, 255, 0.4);
				text-transform: uppercase;
				letter-spacing: 2px;
				margin: 0 0 12px 0;
			}

			.tip-item {
				font-size: 12px;
				color: rgba(255, 255, 255, 0.4);
				padding: 4px 0;
				line-height: 1.5;
			}

			.tip-bullet {
				color: #e50914;
				margin-right: 6px;
			}

			/* Divider */
			.divider {
				height: 1px;
				background: linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.08) 50%, transparent 100%);
				margin: 0 40px;
			}

			/* Footer */
			.footer {
				padding: 24px 40px;
				text-align: center;
			}

			.footer-brand {
				font-size: 14px;
				font-weight: 700;
				color: rgba(255, 255, 255, 0.3);
				letter-spacing: 1px;
				margin: 0 0 8px 0;
			}

			.footer-text {
				font-size: 12px;
				color: rgba(255, 255, 255, 0.2);
				margin: 0 0 4px 0;
				line-height: 1.5;
			}

			.footer-link {
				color: #e50914;
				text-decoration: none;
				font-weight: 600;
			}

			/* Responsive */
			@media only screen and (max-width: 600px) {
				.email-wrapper {
					padding: 16px 0 !important;
				}
				.email-container {
					margin: 0 12px !important;
					border-radius: 12px !important;
				}
				.header {
					padding: 24px 24px !important;
				}
				.brand-name {
					font-size: 22px !important;
				}
				.content {
					padding: 20px 24px 16px 24px !important;
				}
				.password-section {
					padding: 0 24px 8px 24px !important;
				}
				.password-value {
					font-size: 22px !important;
					letter-spacing: 3px !important;
				}
				.steps-section {
					padding: 16px 24px 8px 24px !important;
				}
				.warning-section {
					padding: 12px 24px 24px 24px !important;
				}
				.tips-section {
					padding: 0 24px 22px 24px !important;
				}
				.divider {
					margin: 0 24px !important;
				}
				.footer {
					padding: 20px 24px !important;
				}
				.title {
					font-size: 19px !important;
				}
			}

			@media only screen and (max-width: 400px) {
				.email-container {
					margin: 0 8px !important;
				}
				.header {
					padding: 20px 16px !important;
				}
				.brand-name {
					font-size: 20px !important;
				}
				.content {
					padding: 16px 16px 12px 16px !important;
				}
				.password-section {
					padding: 0 16px 8px 16px !important;
				}
				.password-value {
					font-size: 18px !important;
					letter-spacing: 2px !important;
				}
				.password-card {
					padding: 20px 14px !important;
				}
				.steps-section {
					padding: 12px 16px 8px 16px !important;
				}
				.warning-section {
					padding: 8px 16px 20px 16px !important;
				}
				.tips-section {
					padding: 0 16px 18px 16px !important;
				}
				.divider {
					margin: 0 16px !important;
				}
				.footer {
					padding: 16px 16px !important;
				}
				.title {
					font-size: 17px !important;
				}
				.icon-circle {
					width: 60px !important;
					height: 60px !important;
					line-height: 60px !important;
				}
				.icon-circle span {
					font-size: 26px !important;
				}
			}
		</style>
	</head>
	<body>
		<div class="email-wrapper">
			<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color: #0f0f0f;">
				<tr>
					<td align="center">
						<div class="email-container">

							<!-- Header -->
							<div class="header">
								<p class="brand-name">Cine Circuit</p>
								<p class="brand-tagline">Your Gateway to Cinema</p>
							</div>

							<!-- Icon -->
							<div class="icon-section">
								<div class="icon-circle">
									<span>&#128272;</span>
								</div>
							</div>

							<!-- Content -->
							<div class="content">
								<h1 class="title">Password Reset Successful</h1>
								<p class="subtitle">Your account password has been changed</p>
								<p class="greeting">
									Dear User,<br><br>
									Your password for your <strong style="color: #e50914;">Cine Circuit</strong> account
									has been reset successfully. Below is your new password. Please use it to log in and
									change it to something you can remember.
								</p>
							</div>

							<!-- New Password -->
							<div class="password-section">
								<div class="password-card">
									<p class="password-label">Your New Password</p>
									<p class="password-value">${password}</p>
									<p class="password-hint">Use this password to log in to your account</p>
								</div>
							</div>

							<!-- Next Steps -->
							<div class="steps-section">
								<p class="steps-title">What To Do Next</p>
								<div class="step-item">
									<div class="step-number"><span class="step-badge">1</span></div>
									<div class="step-text">Log in to your account using the new password above</div>
								</div>
								<div class="step-item">
									<div class="step-number"><span class="step-badge">2</span></div>
									<div class="step-text">Go to <strong style="color: rgba(255,255,255,0.8);">Profile Settings</strong> and change your password to something personal and strong</div>
								</div>
								<div class="step-item">
									<div class="step-number"><span class="step-badge">3</span></div>
									<div class="step-text">Do not share your password with anyone for your account safety</div>
								</div>
							</div>

							<!-- Warning -->
							<div class="warning-section">
								<div class="warning-box">
									<p>
										&#9888;&#65039; <strong style="color: rgba(255,255,255,0.65);">Didn't request this?</strong>
										If you did not request a password reset, your account may be at risk. Please
										<a href="mailto:faizankhan901152@gmail.com">contact our support team</a>
										immediately to secure your account.
									</p>
								</div>
							</div>

							<!-- Security Tips -->
							<div class="tips-section">
								<div class="tips-box">
									<p class="tips-title">Security Tips</p>
									<p class="tip-item"><span class="tip-bullet">&#9679;</span> Use a mix of uppercase, lowercase, numbers & symbols</p>
									<p class="tip-item"><span class="tip-bullet">&#9679;</span> Never reuse passwords across multiple websites</p>
									<p class="tip-item"><span class="tip-bullet">&#9679;</span> Never share your password via email, SMS, or phone</p>
								</div>
							</div>

							<!-- Divider -->
							<div class="divider"></div>

							<!-- Footer -->
							<div class="footer">
								<p class="footer-brand">Cine Circuit</p>
								<p class="footer-text">
									Need help? Contact us at
									<a href="mailto:faizankhan901152@gmail.com" class="footer-link">Support</a>
								</p>
								<p class="footer-text" style="margin-top: 12px;">
									&copy; ${new Date().getFullYear()} Cine Circuit. All rights reserved.
								</p>
							</div>

						</div>
					</td>
				</tr>
			</table>
		</div>
	</body>
	</html>`;
};
module.exports = ResetPasswordTemplate;
